...
describe("CBContract", async () => {
  it("can access both accounts", async () => {
    ...
    const [myAccount, whaleAccount] = await getBalance(config.ME_ADDRESS, config.WHALE_ADDRESS);
  ...
  it("Does not transfer if sender does not have the funds available", async () => {
    const { getBalance, transfer, config } = await setupNOKToken();
    const [meBefore, whaleBefore] = await getBalance(config.ME_ADDRESS, config.WHALE_ADDRESS);
    expect(
      transfer({
    ...
    const [meAfter, whaleAfter] = await getBalance(config.ME_ADDRESS, config.WHALE_ADDRESS);
